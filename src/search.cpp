...
void Search::init() {
    ...
    for (int i = 1; i < 2048; ++i)
        cutoffCntReduction[i] = int(std::log(double(i) + 1.186) * 16018.4 / std::log(19.729));
}
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (move == ttMove && (ss + 1)->cutoffCnt < 2)
            r = 0;

        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()]
                      + (*contHist[3])[movedPiece][move.to_sq()] - 4498 
                      - cutoffCntReduction[std::min((ss + 1)->cutoffCnt, 2047)];

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / 15713;
