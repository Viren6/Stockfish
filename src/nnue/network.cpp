...
Network<Arch, Transformer>::evaluate(const Position&                         pos,
                                     ...
                                     StandardFeatureTransformerWeightCache<FTDimensions>* ft_cache) const {
...
void Network<Arch, Transformer>::hint_common_access(
  ...
  AccumulatorCaches::Cache<FTDimensions>* cache,
  StandardFeatureTransformerWeightCache<FTDimensions>* ft_cache) const {
    featureTransformer->hint_common_access(pos, cache, ft_cache);
}
...
Network<Arch, Transformer>::trace_evaluate(const Position&                         pos,
    ...
    for (IndexType bucket = 0; bucket < LayerStacks; ++bucket)
    {
        const auto materialist =
          featureTransformer->transform(pos, cache, transformedFeatures, bucket, nullptr);
        const auto positional = network[bucket].propagate(transformedFeatures);
